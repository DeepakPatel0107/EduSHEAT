...
@WebServlet(name = "UploadCircular", urlPatterns = {"/UploadCircular"})
@MultipartConfig(fileSizeThreshold = 1024 * 1024 * 1, maxFileSize = 1024 * 1024 * 10)
public class UploadCircular extends HttpServlet {
...
//            UpdateProfilePicture.resize(inputImagePath, outputImagePath1, scaledWidth, scaledHeight);
        ...
        String query = "insert into edusheat.circular ( title, date, doc_name, doc_file, doc_path, branch) values('" + title + "','" + date + "','" + fileName + "','" + SAVE_DIR + "','" + filePath + "','" + branch + "')";
        int i = new DBConnection().getQuery(query);
        if (i > 0) {
  out.print("<script>location.href='AdminPages/circulars.jsp';</script>");
