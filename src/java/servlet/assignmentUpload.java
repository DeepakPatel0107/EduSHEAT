...
//            UpdateProfilePicture.resize(inputImagePath, outputImagePath1, scaledWidth, scaledHeight);
        ...
        String query = "insert into edusheat.assignment_upload( regid,type,name,branch, semester, date, doc_name, doc_file, doc_path) values('" + regID + "','" + type + "','" + name + "','" + branch + "','" + semester + "','" + date + "','" + fileName + "','" + SAVE_DIR + "','" + filePath + "')";
        int i = new DBConnection().getQuery(query);
        if (i > 0) {
out.print("<script>location.href='studentsPage/result.jsp';</script>");
         //   response.sendRedirect("http://localhost:8084/SHEAT_Education/FacultyPages/timeTable.jsp");
        } else {
