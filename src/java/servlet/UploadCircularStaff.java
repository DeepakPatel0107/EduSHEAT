...
@WebServlet(name = "UploadCircularStaff", urlPatterns = {"/UploadCircularStaff"})
@MultipartConfig(fileSizeThreshold = 1024 * 1024 * 1, maxFileSize = 1024 * 1024 * 10)
public class UploadCircularStaff extends HttpServlet {

    private static final String SAVE_DIR = "Circular";
    boolean st = false;
...
//            UpdateProfilePicture.resize(inputImagePath, outputImagePath1, scaledWidth, scaledHeight);
        ...
        String query = "insert into edusheat.circular ( title, date, doc_name, doc_file, doc_path, branch) values('" + title + "','" + date + "','" + fileName + "','" + SAVE_DIR + "','" + filePath + "','" + n + "')";
        int i = new DBConnection().getQuery(query);
        if (i > 0) {
  out.print("<script>location.href='FacultyPages/circulars.jsp';</script>");
